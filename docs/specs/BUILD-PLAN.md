...
3. **Pacing is a display state, not an error.** IBKR paces ~60 historical requests per 10 minutes with a 15-second same-contract cooldown. Render values, or `unavailable — pacing limit, retry in 42s`. **`fetching dailies…` is retired as a cell state under `058`** — it contradicted this section's own §7 below (*"fetched or `unavailable (reason)`. There is no third state"*), and §7 wins: a cell is never pending on its own. What renders while an attach's gather is in flight is the ONE screen-level `[ ATTACHING <symbol> ]` state (`SPEC.md` §4, §6b.5) — no per-cell pending state anywhere on the panel.
